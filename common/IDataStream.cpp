 ...
 */
void IDataStream::Skip(UInt32 inBytes)
{
 ...
 */
UInt32 IDataStream::GetLength(void)
{
	return streamLength;
}
 ...
 */
UInt32 IDataStream::GetRemain(void)
{
 ...
 */
UInt32 IDataStream::GetOffset(void)
{
	return streamOffset;
}
 ...
 */
void IDataStream::SetOffset(UInt32 inOffset)
{
...
void IDataStream::CopyStreams(IDataStream * out, IDataStream * in, UInt32 bufferSize, UInt8 * buf)
{
	in->Rewind();
	...
	UInt32	remain = in->GetLength();

	while(remain > 0)
	{
		UInt32	transferSize = remain;
...
void IDataStream::CopySubStreams(IDataStream * out, IDataStream * in, UInt32 remain, UInt32 bufferSize, UInt8 * buf)
{
	bool	ourBuffer = false;
...
IDataSubStream::IDataSubStream(IDataStream * inStream, UInt32 inOffset, UInt32 inLength)
{
	stream = inStream;
	subBase = inOffset;
...
void IDataSubStream::Attach(IDataStream * inStream, UInt32 inOffset, UInt32 inLength)
{
	stream = inStream;
	subBase = inOffset;
	streamLength = inLength;
...
void IDataSubStream::SetOffset(UInt32 inOffset)
{
