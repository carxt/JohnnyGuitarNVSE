...
namespace JohnnyFixes {
	...
	void patchFixDisintegrationsStat() {
		...
		HookUtils::SafeWriteBuf(0x8A1B1B, "\xF9\x01\x74\x12\x83\xF9\x02\x74\x30\x83\xF9\x04\x74\x2B\x83\xF9\x03\x74\x4A\xEB\x22\x90", 22);

		// critical stages 2 and 4, skip IncPCMiscStat
		HookUtils::SafeWriteBuf(0x8A1B6E, "\x82\xC0\x01\x00\x00\xFF\xD0\xEB\x53\x90", 10);
	}
	...
	void Init() {
		...
		HookUtils::WriteRelJump(0x70809E, (uint32_t)InventoryAmmoHook);
		...
		{
			HookUtils::WriteRelCall(addr, uint32_t(GetAmmoEffectsCheckType));
		}

		// fix for companions not saying the next topic after opening ContainerMenu through dialog
		HookUtils::SafeWrite32(0x10721AC, (uint32_t)OnCloseContainerHook);

		// replace fNPCMaxWobbleAngle with 15.0 for VATS
		HookUtils::WriteRelCall(0x8B102B, (uint32_t)VATSSpreadMultHook);
		...
		HookUtils::WriteRelJump(0x88D0D0, (uint32_t)NPCIncrementingChallengesHook);

		// use correct weapon skill req penalty setting in weapon spread calculation
		HookUtils::SafeWriteBuf(0x647902 + 1, "\xC8\xEA\x1C\x01", 4);

		// fix for various biped model update bugs
		HookUtils::WriteRelCall(0x06061E8, (uintptr_t)BipedModelUpdateWeaponHook);
		...
		HookUtils::WriteRelCall(0x5BED66, (uint32_t)GetINISettingHook);
		HookUtils::WriteRelCall(0x5BEF13, (uint32_t)GetINISettingHook);
		HookUtils::WriteRelCall(0x5B6C80, (uint32_t)SaveINIHook);

		HookUtils::WriteRelJump(0x5BED86, (uint32_t)GetINISettingTypeHook);

		// fixes for null pointers when showing credits outside of start menu
		HookUtils::WriteRelCall(0x75F770, (uint32_t)MenuGetFlagHook);
		HookUtils::WriteRelCall(0x75F8AE, (uint32_t)MenuSetFlagHook);
		HookUtils::WriteRelCall(0x75F6DA, (uint32_t)MenuSetFlagHook);

		// missing nullcheck in HandleStealing
		HookUtils::SafeWriteBuf(0x8BFBC1, "\x85\xC9\x74\x36\x80\x79\x04", 7);

		// fix for incidental sounds not working in regions
		HookUtils::WriteRelCall(0x4F49AB, uint32_t(TESRegionDataSoundIncidentalIDHook));

		// fix for the stimpak crash
		HookUtils::WriteRelCall(0x7DB525, (uintptr_t)StimpakHotkeyHook);
		...
		HookUtils::WriteRelJump((uintptr_t)0x0490B10, (uintptr_t)0x0490B41);

		// fix NPE in BSTempEffectSimpleDecal
		HookUtils::WriteRelJump(0x68D2EB, (uint32_t)SimpleDecalHook);


		// Fix for missing baseprocess null check
		HookUtils::WriteRelJump(0x59DF40, (uint32_t)IsCurrentFurnitureRefHook);

		//AnimData NPEs
		//fix NPE in AnimData freeing
		HookUtils::WriteRelJump(0x0490BBB, (uintptr_t)AnimDataNullCheck);
		HookUtils::WriteRelCall(0x0A2EC64, (uintptr_t)NiContManNullCheck2);

		CloudUpdateFix::Install();

		//Stop game from crashing on extensions reeeeeeeeeee
		HookUtils::WriteRelCall(0x83509D, (uintptr_t)fixAudioMonoLookupOverflow);
		...
		HookUtils::WriteRelCall(0x5B7812, (uint32_t)SetCellImageSpaceHook);

		//Patch the game so the dialog subroutine stops if the actor's head is blown off, I'll add it as an ini setting later.
		HookUtils::WriteRelCall(0x8EC54D, (uintptr_t)NoHeadlessTalkingHook);
	}
