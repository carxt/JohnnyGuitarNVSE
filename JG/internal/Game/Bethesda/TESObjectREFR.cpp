...
#else
	const ExtraDataList* pExtra = GetExtra();
	return GetObjectReference() == apObject
		|| pExtra->GetLevCreaOriginalBase() == apObject
		|| pExtra->GetLevCreaTemplate() == apObject;
#endif
}
...
#else
	if (pLoadedData)
		return pLoadedData->fRelevantWaterHeight;
	else
...
#else
	if (pLoadedData)
		return pLoadedData->pCurrentWaterObject;
	else
		return nullptr;
#endif
}
...
#else
	NiAVObject* pRoot = Get3DSimple();
	if (pRoot) {
		pRoot->SetLocalTranslate(GetPosition());
		pRoot->SetLocalRotate(GetOrientation());
		pRoot->Update();
	}
#endif
}
...
#else
	return GetExtra()->GetIgnoredBySandbox();
#endif
}
...
#ifdef GAME
	ThisCall(0x564DB0, this, abVal);
#else
	ThisCall(0x63D930, this, abVal);
#endif
}
...
#ifdef GAME
// GAME - 0x56A250
bool TESObjectREFR::IsActivatingChildren() const {
...
#else
	return pRandomSound && pRandomSound->GetData().uiFlags.bPlayAtRandom;
#endif
}
...
void TESObjectREFR::SetEmittanceSource(TESForm* apSource) {
	GetExtra()->SetEmittanceSource(apSource);
}
...
// GAME - 0x563650
void TESObjectREFR::SaveAnimation(BGSSaveFormBuffer* apBuffer) { 
	ThisCall(0x563650, this, apBuffer); 
}

// GAME - 0x5636E0
void TESObjectREFR::LoadAnimation(BGSLoadFormBuffer* apBuffer) { 
	ThisCall(0x5636E0, this, apBuffer); 
}
#endif
