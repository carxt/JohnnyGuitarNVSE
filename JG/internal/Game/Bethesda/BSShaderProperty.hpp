...
public:
	...
	public:
		...
		void AddPass(NiGeometry* apGeometry, uint32_t auiPassEnum, bool abFirst,
			uint8_t aucNumLights = 0, ShadowSceneLight* apSceneLight = nullptr, ShadowSceneLight* apSceneLight1 = nullptr, ShadowSceneLight* apSceneLight2 = nullptr, ShadowSceneLight* apSceneLight3 = nullptr);

		void Clear();

		bool IsEmpty() const;

		RenderPass* GetLastPass() const;

		RenderPass* GetAt(uint32_t auiIndex) const;
	};
	...
	void SetFadeAlpha(float afAlpha);

	float GetEnvmapScale() const;
	void SetEnvmapScale(float afScale);

	float GetCameraDistance() const;

	void ClearRenderPasses();

	void SetLastRenderPassState(int32_t aiState);
	...
	bool GetSkipNormalMaps() const { return ullFlags.bSkipNormalMaps; }
