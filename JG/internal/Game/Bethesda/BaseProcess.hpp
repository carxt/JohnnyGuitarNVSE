...
public:
	...
	struct _PostAnimAction {
		enum Action {
			...
			PLAY_IDLE					= 1u << 4,
			DIE							= 1u << 5,
			EQUIP_UNEQUIP_WEAPON		= 1u << 6,
			PICK_IDLE_CALL				= 1u << 7,
			HAND_GRIP_WEAPON			= 1u << 8,
			GET_OUT_OF_FURNITURE		= 1u << 9,
			FORCE_IDLE_ANIMATION		= 1u << 10,
			FREE_SPECIAL_IDLE			= 1u << 11,
			RELOAD_TARGETS				= 1u << 12,
			IDLE_LOADED					= 1u << 13,
			HAVOK_WEAPON				= 1u << 14,
			BLOCK						= 1u << 15,
			SET_IRON_SIGHTS				= 1u << 16,
			EXPEL_SHELLCASING			= 1u << 17,
			RELOAD_SINGLE				= 1u << 18,
			START_LOOPING_RELOAD		= 1u << 19,
		};
	};
	using PostAnimAction = _PostAnimAction::Action;

	struct _DetectionListType {
		enum Type {
			BY_ME		= 0,
			BY_OTHERS	= 1,
		};
	};
	using DetectionListType = _DetectionListType::Type;
	...
	using FadeState = _FadeState::State;

	struct _HeadTrackType {
		enum Type : uint32_t {
			DEFAULT		= 0,
			ACTION		= 1,
			COMBAT		= 2,
	...
	using HeadTrackType = _HeadTrackType::Type;
	...
	virtual NiPoint3						GetLastPositionDetected(Actor* apActor, DetectionListType aeType);
	virtual double							GetLastTimeDetected(Actor* apActor, DetectionListType aeType);
	...
	virtual bool							SetupSpecialIdle(Actor* apActor, TESIdleForm* apIdle, ANIM_IDLE_PLAY_TYPE aeDefaultAction, bool abCrippled, bool abForce, bool abTestConditions);
	...
	virtual void							PerformOneRoundofCombat(Actor* apActor, int32_t aiRounds);
	...
	virtual bool							ShouldUpdateCombatControllerOrEndCombat(Actor* apActor, TESPackage*& apPackage);
	...
	virtual DetectionState*					InsertIntoDetectionList(Actor* apActor, DETECTION_LEVEL aeDetectionLevel, bool abLineSight, int32_t aiLevel, DetectionListType aeType, bool ab360View, bool abInCombat);
	virtual BSSimpleList<DetectionState*>*	GetDetectingActors();
	virtual DetectionEvent*					GetActorsDetectionEvent(Actor* apActor);
	virtual void							SetActorsDetectionEvent(Actor* apActor, NiPoint3 akPosition, SOUND_LEVEL aeSoundLevel, int32_t aiDetectionEventType, TESObjectREFR* apRef);
	...
	virtual void							AddQueuedItem(Actor* apActor, bool abEquip, TESBoundObject* apItem, int32_t aiNumber, ExtraDataList* apExtraList, bool abNow, bool abForceEquip, bool abFromMenus, bool abForceUnequip, bool abEquipCalling, bool abPlaySound);
	...
	virtual WadingWaterData*				GetWadingWaterData() const;
	virtual void							RemoveDetectionActor(Actor* apActor, DetectionListType aeType);
	virtual void							SetDetectionTimer();
	virtual void							GetLineSightonActor(Actor* apActor, Actor* apTarget, DetectionListType aeType, bool abFullTest);
	virtual bool							Get360LineSightonActor(Actor* apActor, Actor* apTarget, DetectionListType aeType);
	virtual int32_t							GetDetectionActor(Actor* apTarget, DetectionListType aeType) const;
	...
	virtual void							SetLastSpeakingEmotion(DIALOGUE_EMOTION aeEmotion);
	...
	virtual ItemChange*						CreateWeaponAcquireList(Actor* apActor, PACKAGE_OBJECT_TYPE aePackageObjectType);
	...
	virtual void							Move(MobileObject* apObject, NiPoint3 akSideVector);
	...
	virtual void							SetSitSleepState(Actor* apActor, SIT_SLEEP_STATE aeState, TESObjectREFR* apFurniture, uint8_t aucFurnitureIndex);
	...
	virtual DetectionState*					GetDetectionState(Actor* apTarget, DetectionListType aeType);
	virtual DetectionState*					InsertIntoDetectionArray(Actor* apTarget, DETECTION_LEVEL aeDetectionLevel);
	...
	virtual void							SetEndIdlesPlayed(bool abVal);
	...
	virtual void							SetHasHealingPotions(int32_t aiHasHealingPotion);
	virtual int32_t							GetHasHealingPotions() const;
	virtual float							GetTemporaryModifier(ActorValue::Index aeActorValue);
	virtual float							GetPermanentModifier(ActorValue::Index aeActorValue);
	virtual float							GetDamageModifier(ActorValue::Index aeActorValue);
	...
	virtual FadeState						GetFadeState() const;
	virtual void							AddPostAnimationAction(PostAnimAction aeFlag);
	virtual uint32_t						GetPostAnimationActions() const;
	virtual void							ClearPostAnimationActions();
	virtual void							RemovePostAnimationAction(PostAnimAction aeFlag);
	...
	virtual void							StoreDismemberedLimbNode(LIMB_TYPE aeBodyPart, NiNode* apNode);
	virtual NiNode*							GetDamageNode(LIMB_TYPE aeBodyPart) const;
	...
	virtual void							SetAnimActionSuccess(int32_t aiValue);
	...
	virtual void							DeferHideDismemberedLimb(LIMB_TYPE aeLimbIndex, NiNode* apDismemberedLimbRoot, NiAVObject* apReplacementLimb, bool abExplosion);
	...
	virtual int32_t							GetWeaponConditionStage() const;
	virtual void							SetWeaponConditionStage(int32_t aiStage);
	...
	PROCESS_LEVEL	eProcessLevel;
	CachedValues*	pCachedValues;

	PROCESS_LEVEL GetProcessLevel() const;
